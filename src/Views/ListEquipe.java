 ...
 * @author bhk
 */
public class ListEquipe extends Form {
    private Resources theme;
    Form current;

    public ListEquipe(Form previous, Resources res) {
        setTitle("Equipe Liste");

//        SpanLabel sp = new SpanLabel();
//        sp.setText(MenuService.getInstance().getAllMenus().toString());
//        add(sp);

        ArrayList<Equipe>list = EquipeController.getInstance().getAllEquipes();
    ...
    private void showEquipe( Equipe m,Form previous, Resources res) {
        ...
        Label lbDescription = new Label("Classement : "+m.getClassement(),"NewsTopLine2");
        ...
        lbDelete.addPointerPressedListener(l->{
            ...
            if(dig.show("Suppression","Vous voulez supprimer ce menu?", "Annuler","Oui")){
                ...
                if(EquipeController.getInstance().deleteEquipe(m.getId())){
        ...
        lbUpdate.addPointerPressedListener(l->{
            new ModifierEquipe(previous, res, m).show();
        });
        
        cnt.add(BorderLayout.CENTER, BoxLayout.encloseY(BoxLayout.encloseX(lbTitre, lbDelete, lbUpdate), BoxLayout.encloseX(lbDescription),BoxLayout.encloseY(inscrit)));
        add(cnt);
    }
